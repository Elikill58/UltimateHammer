...
public enum Tree {
	...
	Tree(Version minVersion, List<String> trunk, List<String> leaves, boolean diagonalLeave) {
		...
		if(Version.getVersion().isNewerOrEquals(minVersion)) {
			this.trunk = trunk.stream().map(Material::valueOf).collect(Collectors.toList());
			this.leaves = leaves.stream().map(Material::valueOf).collect(Collectors.toList());
		} else {
	...
	Tree(Version minVersion, String name) {
		this.minVersion = minVersion;
		if(Version.getVersion().isNewerOrEquals(minVersion)) {
			this.trunk = Arrays.asList(Material.valueOf(name + "_LOG"), Material.valueOf(name + "_WOOD"));
			this.leaves = Arrays.asList(Material.valueOf(name + "_LEAVES"));
		} else {
			this.trunk = new ArrayList<>();
			this.leaves = new ArrayList<>();
		}
