...
public class Paper1_21Version extends SpigotVersionAdapter {
	...
	public Paper1_21Version(String version) {
		...
		if (v.isNewerOrEquals(Version.V1_18)) {
			...
			if(v.equals(Version.V1_21)) {
				getTag = ReflectionUtils.getMethodNamed(nmsItemStackClass, "d");
				saveNbt = ReflectionUtils.getMethodNamed(nmsItemStackClass, "saveOptional");
				//getTagParams = new Object[] { null };
			} else {
		...
		if(issues.length() > 0)
			Adapter.getAdapter().getLogger().warn("Methods " + issues.toString() + " have not been found.");
	}
	...
	private Object getNBTTagCompoundFromNMSItem(Object nmsItem) throws Exception {
		if (nmsItem == null)
			return null;
		Object tag = getTag.invoke(nmsItem, getTagParams);
